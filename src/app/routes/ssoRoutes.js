...
module.exports = (app) => {
  ...
  app.get('/v1/sso/contact/verified', async (req, res) => {
    ...
    try {
      ...
      redirectUrl = successUrl + getQueryParams({
  ...
  app.get(successUrl, async (req, res) => { // to support mobile sso flow
    ...
    res.status(200).sendFile('./success_loader.html', {
      root: __dirname
    })
  });
  ...
  app.get('/v1/sso/create/session', async (req, res) => { // needs to onboard to kong
    ...
    try {
      ...
      response = {
        error: getErrorMessage(error, errType)
      };
  ...
  app.get(errorUrl, (req, res) => {
    res.status(200).sendFile('./error_loader.html', {
      root: __dirname
    })
  })
...
const logErrorEvent = (req, type, error) => {
  ...
  telemetryHelper.logApiErrorEventV2(req, {
    edata,
    context
  });
}
