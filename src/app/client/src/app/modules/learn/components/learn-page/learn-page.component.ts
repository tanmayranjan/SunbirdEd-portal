...
export class LearnPageComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    this.userService.userData$.pipe(first()).subscribe((user: IUserData) => {
      if (user && !user.err) {
  ...
  private fetchPageData() {
    ...
    if (this.queryParams.sort_by) {
      option.sort_by = {
        [this.queryParams.sort_by]: this.queryParams.sortType
      };
    }
      ...
      .subscribe(
        data => {
          ...
          if (this.enrolledSection.contents.length) {
            ...
            !this.enrolledSection.contents.length &&
            this.carouselMasterData.length >= 2
          ) {
            this.pageSections = [
              this.carouselMasterData[0],
              this.carouselMasterData[1]
            ];
          } else {
            this.pageSections = [this.carouselMasterData[0]];
          }
  ...
  private prepareCarouselData(sections = []) {
    const {
      ...
      dynamicFields,
      slickSize
    } = this.configService.appConfig.CoursePageSection.course;
    const carouselData = _.reduce(
      sections,
      (collector, element) => {
        const contents =
    ...
    return this.coursesService.enrolledCourseData$.pipe(
      map(({ enrolledCourses, err }) => {
        ...
        enrolledSection.contents = _.map(enrolledCourses, content => {
          const formatedContent = this.utilService.processContent(
            content,
            constantData,
            dynamicFields,
            metaData
